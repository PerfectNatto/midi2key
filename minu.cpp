...
int ring_buffer_init(RingBuffer* rb, size_t size) {
    ...
    rb->read_pos = 0;
    rb->write_pos = 0;
    if (ma_mutex_init(&rb->mutex) != MA_SUCCESS) return -1;
    if (ma_cond_init(&rb->cond) != MA_SUCCESS) return -1;
    return 0;
}
...
size_t ring_buffer_write(RingBuffer* rb, const float* data, size_t frames) {
    ...
    ma_cond_signal(&rb->cond);
    ma_mutex_unlock(&rb->mutex);
